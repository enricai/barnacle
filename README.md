...
| `SUBMISSIONS_NDJSON_PATH` | `.barnacle/submissions.ndjson` | Append-only NDJSON sink for dispatch submission envelopes. One line per plugin invocation captures siteId, requestId, inbound payload, status, audit payload, and duration, plus the named `vivclid`/`jobReference` reconciliation keys and a `kind:"submit"` dimension (null/`"submit"`-defaulted on legacy lines) — the durable source-of-truth for "what did we submit for jobId X and did it succeed," joinable to the Appcast CPA report without re-parsing `inboundPayload`. |
