...
Drives a real Stagehand + Steel browser through your flow. Wires response listeners on both context and page level so no early requests are missed (some requests fire before a page-level handler is registered). Captures every network call matching `/graph`, `/api/`, `/graphql`, `/v1/`, or `*.json` to `/tmp/recon/graphql/<NNN>-<phase>-<operationName>.json` — one file per call, diffable and greppable. Use `--capture-all` for sites with non-standard API paths — it captures all responses, producing more noise but missing nothing. Total runtime: ~90 seconds, fully unattended.
...
export const SITE_PLUGINS: SitePlugin<unknown, unknown>[] = [
  ...
  mySitePlugin as SitePlugin<unknown, unknown>,  // add your plugin here
];
```
...
**90-second hard timeout:** each queued task (one plugin invocation) races against a 90s timer. This is a per-task timeout, not a per-session lifetime — a single `execute()` call that hangs (frozen CDP connection, infinite network wait) converts to `SessionTimeoutError`, which the retry policy handles by restarting the session and retrying.
...
The `[1m]` suffix selects the 1-million-token context variant on Bedrock. Both paths enable Stagehand's action cache (`enableCaching: true`).
...
BARNACLE_SITE_EXAMPLE_BASE_URL="https://sandbox.example.com"  # overrides plugin `example`
```
---
...
With the dev server running (`pnpm run dev`) and `DEV_BYPASS_AUTH=true` in `.env`, call the built-in example plugin:

```bash
curl -s -X POST http://localhost:3000/v1/example/run \
  -H "Content-Type: application/json" \
  -d '{"query": "test"}' | jq .
```

Expected response shape (the `data` field comes from the plugin's `responseSchema`):
...
**Bypass header:** send `X-Barnacle-Force-Fallback: true` on any plugin request to skip the hot path and go directly to the Stagehand browser path. Useful for debugging or when you know the hot path is broken.
