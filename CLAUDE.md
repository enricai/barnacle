...
- Build: `pnpm run build` (tsc → dist/ via tsconfig.build.json + tsc-alias path rewriting + copies every `src/**/fixtures/` dir into dist/)
...
- **ALWAYS** use `async/await` over `.then()`. Exception: `.then()/.finally()` is acceptable when `await` would yield before a synchronous invariant is established (e.g. registering an in-flight promise before any other caller can race in).
