...
**Total runtime: ~90 seconds, fully unattended.**

### 1a — Session bootstrap

The script calls `createBrowserSession()` (`src/scraper/session.ts`), which
hardcodes `enableCaching: true` when constructing the Stagehand instance.
Stagehand's built-in action cache skips LLM inference on replay — after the
first run against a page structure, subsequent `act()` calls resolve from cache
in milliseconds.

### 1b — Dual-level response listeners

Listeners are registered on both the browser context and the page:
...
This matters because some requests fire during navigation, before a page-level
handler is wired up. The context-level listener catches those early requests.
Without it, the first call in a flow is silently missed.
...
### 1c — AI-driven flow execution

Stagehand executes each natural-language instruction autonomously. The recon
script calls `page.act()` — Stagehand's proxy attaches `act()` directly onto
the Playwright page object:

```ts
await page.goto(url, { waitUntil: "networkidle" });
await page.act("click the Electronics category filter");
await page.act("open the first product result");
```

Claude resolves DOM selectors from the natural-language description. Its only
job here is triggering the SPA's network traffic — not extracting DOM data.
This is the whole point of the approach: you describe what a user *does*,
not which CSS selector to click.

### 1d — Per-call capture files
...
Cache key = `<endpoint>:<sha256(canonical payload)[:32]>` — the endpoint is a
literal prefix separated by a colon; the hash covers only the canonical payload.
Object key order and primitive array order are normalized so `{a:1,b:2}` and
`{b:2,a:1}` hit the same entry. Default TTL: 15 minutes. Configurable via
`CACHE_TTL_MS`.
...
Hot path fails (schema mismatch, bot challenge, or 5xx)
  ...
  → runWithSession(fn)                          [src/scraper/pool.ts]
    ...
    → withScraperRetry (up to 3 attempts)       [wraps everything below]
      ...
      → Promise.race([plugin.execute(session), 90s timeout])
...
Useful for debugging or when you know the hot path is broken.
...
| `SessionTimeoutError` | 90s pool timeout | Kill session → create fresh → retry up to `maxAttempts` (restart happens at most once) |
...
The **90-second hard timeout** per task prevents a hung Stagehand operation
(infinite network wait, frozen CDP connection) from blocking a queue slot
indefinitely. The timeout converts a silent hang into `SessionTimeoutError`,
which the retry policy can act on by restarting the session.
...
cache miss triggers Claude to re-resolve the selector. The fallback self-heals
for small UI changes without human intervention.
...
`recon:browser` reruns in ~90 seconds unattended. Auto-captured results are
diffable against prior runs; human memory is not.
