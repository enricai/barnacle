...
An `emailStep` marks a genuine external-dependency step. The generator should emit a browser-flow that calls the same inbox-poll + extract + act sequence (the plugin equivalent of the hook), using the engine's `pollTestmailInbox` — **not** hardcode a link. For a production plugin the inbox is the **end user's real inbox**, not testmail, so the generated code must take the mailbox source from config/payload, not assume testmail. This is the difference between a recon-only capability and a shippable one — worth a design note even though v1 only supports the testmail path for recon.
