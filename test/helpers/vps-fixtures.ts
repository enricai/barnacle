...
// Source of truth is the RC-shipped docs directory. We used to keep a
// duplicate copy under test/fixtures/vps/ but it drifted risk was real
// and zero upside — the one canonical file is the one RC shipped us.
const FIXTURE_DIR = join(process.cwd(), "RC_API_Docs");
...
function extractJsonObjects(src: string): string[] {
  ...
  if (depth !== 0) throw new Error("unbalanced braces in fixture");
  return out;
}
