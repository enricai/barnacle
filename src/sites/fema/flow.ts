...
async function phase1PreApplication(
  ...
  await session.stagehand.page.goto(baseUrl);
...
async function phase4ApplicationCenter(
  ...
  // Patch all required sections to 'complete' via window.femaMock.setState, which
  // updates the in-memory applicationState AND persists to localStorage in one call.
  // Patching localStorage alone doesn't work because router.navigate() calls
  // saveState() (in-memory → localStorage) BEFORE rendering the review page,
  // which would overwrite any localStorage-only patch.
  // window.femaMock is exposed by app.js for debugging and is available in the
  // browser context on the mock site.
  await session.limiter.schedule(() =>
    session.stagehand.page.evaluate(`
      (() => {
        if (!window.femaMock?.setState) return;
  ...
  try {
    if (input.sessionFixture) {
      ...
      await session.stagehand.page.goto(
        `${baseUrl}?sessionFixture=${encodeURIComponent(input.sessionFixture)}`
      );
      pagesCompleted = 22;
    } else {
      await phase1PreApplication(session, input.preApplication, baseUrl);
      pagesCompleted = 6;
