...
// The querystring schema `.transform()`s a stringy body into the same
// SailingPackageRequest shape the POST /v1/search route produces, so the
// inferred type is already the service's input — no cast needed.
type Querystring = z.infer<typeof sailingPackageQueryStringSchema>;
...
export async function sailingPackageRoute(app: FastifyInstance): Promise<void> {
  app.get<{ Querystring: Querystring }>(
    ...
    async (request) => {
      return getSailingPackages(request.query);
    }
  );
}
