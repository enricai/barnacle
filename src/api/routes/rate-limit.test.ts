...
describe("inbound rate limiting (Task 11)", () => {
  ...
  it("emits a VPS envelope with code 1010 after the limit is crossed, plus X-RateLimit-* headers", async () => {
    ...
    // README documents X-RateLimit-* + Retry-After as the contract. Pin the
    // @fastify/rate-limit defaults so an accidental `addHeaders: false` or
    // a migration to the `enableDraftSpec` variant breaks this test first
    // instead of silently breaking clients that branch on these names.
    expect(limited.headers["retry-after"]).toBeDefined();
    expect(limited.headers["x-ratelimit-limit"]).toBeDefined();
    expect(limited.headers["x-ratelimit-remaining"]).toBe("0");
    const ok = responses.find((r) => r.statusCode === 200);
    if (!ok) throw new Error("no 200 response in the sample — fixture drift");
    expect(ok.headers["x-ratelimit-limit"]).toBeDefined();
    expect(ok.headers["x-ratelimit-remaining"]).toBeDefined();
  });
});
