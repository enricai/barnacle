...
const bestRateSchema = z
  .object({
    ...
    leadPromotion: leadPromotionSchema.nullable().optional(),
 ...
 */
const bestPriceEnvelopeSchema = z
  .object({
...
/**
 * Shared `metaInfo` block RC echoes on every pricing response — reports the
 * bookingChannel and the effective cutover timestamp. `effectiveDateTime` is
 * a YYYYMMDDHHMMSS numeric on the wire, accepted as either form to absorb a
 * future migration to ISO strings.
 */
export const pricingMetaInfoSchema = z
  .object({
    bookingChannel: z.string().optional(),
    effectiveDateTime: z.union([z.number().int(), z.string()]).optional(),
  })
  .passthrough();

/**
 * `{brochure,standard}BestRate` pair used by both super-category and
 * category base-price responses. Keyed differently at the wrapping level
 * (stateroomSuperCategory vs stateroomCategoryCode) so the outer entries
 * remain per-endpoint.
 */
export const brochureStandardRatePairSchema = z
  .object({
    brochureBestRate: bestRateSchema.optional(),
    standardBestRate: bestRateSchema.optional(),
  })
  .passthrough();
