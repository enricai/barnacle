/**
 * Acceptance coverage for the replan-splice captchaGated retention fix
 * (`applyFailedStepFlagsToResumingBridgeStep`, `src/scripts/recon-browser.ts`):
 * the replan prompt never asks the LLM to carry `captchaGated`/`submitStep`
 * forward, and the original failed step object is discarded at splice time,
 * so retention used to depend entirely on what the mocked replan LLM
 * happened to return. This drives `main()` for real (only
 * `createBrowserSession` and the replan LLM call are stubbed) with a
 * `captchaGated: true` step that fails its cascade, has the replan LLM
 * propose a bridge step for the SAME control action with NO captchaGated
 * flag set at all, and asserts the next `executeStepWithHealing` call — for
 * the spliced-in bridge step — still carries `captchaGated: true`
 * deterministically, not by LLM inference.
 *
 * This is a sibling of
 * `src/scripts/recon-browser.replan-captchagated-flag-retention.test.ts`,
 * which exercises the identical fix via the recon-browser entry point; kept
 * separate (not merged/moved) because the two live at different artifact
 * registry paths.
 */
...
const BASE_URL = "https://portal.example.org/app/onboarding";
const ORIGINAL_STEP = "Click the 'Continue' button";
const BRIDGE_STEP = "Solve the challenge, then click the 'Continue' button again to proceed";
...
function makeFakePage(): { page: Page; stagehand: Stagehand } {
  ...
  const page = {
    ...
    url: (): string => BASE_URL,
    title: vi.fn().mockResolvedValue("Onboarding"),
...
describe("flow-runner — captchaGated flag retention across a global replan splice", () => {
  const ORIGINAL_ARGV = process.argv;
  let runsRoot: string;

  beforeEach(() => {
    runsRoot = mkdtempSync(join(tmpdir(), "flow-runner-replan-captchagated-"));
  ...
  it("re-applies captchaGated:true to the spliced bridge step even though the mocked replan LLM output carried no flag at all", async () => {
    ...
    // that re-executes against the same "Continue" control still carries it.
