...
interface SitemapItineraryEntry {
  url: string;
  packageCode: string;
  shipCode: string;
...
export async function fetchSitemapItineraries(
  ...
  try {
    ...
    if (name === "TimeoutError" || name === "AbortError") {
      logger.warn(`itinerary sitemap fetch timed out after ${config.scraper.httpTimeoutMs}ms`);
      throw new Error(`itinerary sitemap fetch timed out after ${config.scraper.httpTimeoutMs}ms`);
    }
    throw err;
  }
  if (!response.ok) {
