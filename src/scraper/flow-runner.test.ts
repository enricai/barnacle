...
describe("flow-runner/snapshotPage — post-action url read on a stale child frame", () => {
  const IFRAME_SELECTOR = "iframe#apply_frame";
  const STALE_FRAME_URL = "https://apply.example.com/application/abc-123/step-1";
  const NEW_FRAME_URL = "https://apply.example.com/application/abc-123/step-2";
  const TOP_WRAPPER_URL = "https://careers.example.org/jobs/123";

  /** Minimal fake `Frame`: only what `tryResolveChildFrame`'s re-resolution probe reads. */
  ...
  /** Minimal fake `Page`: answers `probeAttachedFrameTarget`'s iframe-src probe and `frames()`. */
  function makeFakePage(newFrame: ReturnType<typeof makeFakeFrame>) {
    return {
      url: () => TOP_WRAPPER_URL,
      ...
      evaluate: async (expr: unknown) => {
        ...
        const selector = match?.[1] ? (JSON.parse(match[1]) as string) : null;
        if (selector !== IFRAME_SELECTOR) return { matched: false, src: null };
        return { matched: true, src: NEW_FRAME_URL };
      },
  ...
  it("credits urlChanged against the re-resolved child frame's NEW url, not the top wrapper page's url, when the frame handle goes stale post-action", async () => {
    const preActionTarget: FrameTarget = {
      frame: makeFakeFrame(STALE_FRAME_URL) as never,
      frameSelector: IFRAME_SELECTOR,
      declaredFrameSelector: IFRAME_SELECTOR,
      evaluate: async <R = unknown>() => null as R,
      locator: (selector: string) => ({ scope: "frame" as const, selector }) as never,
      url: () => Promise.resolve(STALE_FRAME_URL),
      title: async () => "unused",
    };
    // Same declared selector, but the in-frame navigation the action triggers
    // detached this Frame handle — its own url() now rejects, the exact
    // failure mode readCurrentFrameUrl re-resolves around instead of falling
    // back to the top wrapper page's url.
    const postActionTarget: FrameTarget = {
      ...preActionTarget,
      url: () => Promise.reject(new Error("execution context was destroyed")),
    };
    const page = makeFakePage(makeFakeFrame(NEW_FRAME_URL));

    const pre = await snapshotPage(preActionTarget, { n: 0 }, page as never);
    const post = await snapshotPage(postActionTarget, { n: 0 }, page as never);
    const urlChanged = pre.url !== post.url;

    expect(pre.url).toBe(STALE_FRAME_URL);
