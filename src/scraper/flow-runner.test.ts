...
describe("flow-runner/pollEnumerate", () => {
  it("evaluates against page when no evalTarget is given", async () => {
    const { page } = fakePage([1]);
    const result = await pollEnumerate<number>(page, "1", (n) => n > 0);
    expect(result).toBe(1);
    expect(page.evaluate).toHaveBeenCalledTimes(1);
  });

  it("evaluates against the resolved FrameTarget, not the main page, when evalTarget is given", async () => {
    ...
    const result = await pollEnumerate<number>(page, "1", (n) => n > 0, { evalTarget: target });
    expect(result).toBe(1);
