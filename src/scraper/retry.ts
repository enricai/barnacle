  ...
  return pRetry(
    async () => {
      try {
        ...
        if (err instanceof CaptchaError || err instanceof EmptyResultsError) {
    ...
    },
    {
      ...
      onFailedAttempt: async (error) => {
        const wrapped = error as unknown as ScraperError;
        logger.warn(
          `scraper attempt ${error.attemptNumber} failed (${wrapped.name}): ${error.message}; ${error.retriesLeft} retries left`
        );
        if (options.onRetry && wrapped instanceof ScraperError) {
