...
/**
 * Regression for bugfix-004: a click resolving to a disabled (or
 * `aria-disabled="true"`) target must never be scored verified, even when a
 * weak page-wide DOM-delta signal (htmlDelta/textChanged from a validation
 * re-render) is present. Matches the exact recon report shape:
 * `#enterEmailSubmitButton` disabled, htmlDelta=52, previously scored
 * verified=true. Covers both `verifyDomEffect` (the primary click verifier)
 * and the n+16 `el.click()` fallback inside `executeStepWithHealing`.
 */
...
/** First step matches the root itself (`<html>`); the rest walk into children. */
...
/**
 * Builds an `<html><body><div>` tree with a submit button that is either
 * natively `disabled` or carries `aria-disabled="true"` — the exact two
 * forms the recon report and the required veto both name. `bodyRevision`
 * grows every time the button's `click()` fires, modeling the validation
 * re-render (banner insertion) whose htmlDelta/textChanged signal used to
 * ride past the disabled target.
 */
function buildTree(kind: "disabled" | "aria-disabled"): {
  ...
  const button = makeElement("button", {
    disabled: kind === "disabled" ? true : undefined,
    onClick: () => {
      bodyRevision.n += 1;
    },
  });
  if (kind === "aria-disabled") {
    button.attrs["aria-disabled"] = "true";
  }
  appendChild(container, button);
  return { htmlRoot: html, button, bodyRevision };
}
...
describe("flow-runner — disabled/aria-disabled click target veto", () => {
  it.each([["disabled" as const], ["aria-disabled" as const]])(
    "verifyDomEffect never scores a click on a %s target as verified",
    async (kind) => {
      const { htmlRoot } = buildTree(kind);
      const selector = "xpath=/html[1]/body[1]/div[1]/button[1]";
      ...
      const verified = await verifyDomEffect(target, {
        selector,
        method: "click",
        description: "enterEmailSubmitButton",
        arguments: [],
      });
  ...
  it.each([["disabled" as const], ["aria-disabled" as const]])(
    "n+16 el.click() fallback never scores a click on a %s target as verified despite a nonzero htmlDelta",
    async (kind) => {
      const { htmlRoot, bodyRevision } = buildTree(kind);
      const selector = "xpath=/html[1]/body[1]/div[1]/button[1]";

      guardedObserve.mockResolvedValue([
        { selector, description: "enterEmailSubmitButton", method: "click" },
      ]);
      guardedAct.mockResolvedValue({
        success: true,
        message: "clicked",
        actionDescription: "enterEmailSubmitButton",
        actions: [{ selector, description: "enterEmailSubmitButton", method: "click" }],
      });
      ...
      await expect(
        executeStepWithHealing({
          ...
          page,
          step: "Click the enter email submit button",
          optional: false,
          upload: false,
          submitStep: false,
          stepIndex: 0,
      ...
      // Proves the fallback actually clicked the disabled target and grew the
      // page (htmlDelta > 0 on every retry attempt) — the exact weak signal
      // the recon report captured as a false verified=true — yet never
      // credited the step.
      ...
      for (const line of n16Logs) {
        expect(line).toMatch(/htmlDelta=(?!0\s)\d+/);
