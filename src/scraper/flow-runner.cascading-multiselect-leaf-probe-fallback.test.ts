...
/**
 * Regression for bugfix-001: the existing cascading-multiselect tests all
 * drive `executeStepWithHealing` with "select 'X'" phrasing, which was never
 * the failing shape — the recon doc's actual flow-generation text for the
 * category/leaf clicks is "click the top-level category 'X' to expand its
 * sub-options" / "click the leaf option 'Y' to commit the source value".
 * That click+category/option phrasing is exactly what `parseWidgetOptionClickStep`
 * was added to recognize, so this test drives the primitive with the report's
 * literal wording to prove the real gap stays closed.
 */
 ...
 * categories; clicking a category option swaps the SAME popup to that
 * category's leaves without committing the widget; clicking a leaf option
 * commits the widget's value and clears its invalid marker.
 */
...
describe("flow-runner/tryPromptSelectorPrimitive cascading-multiselect click phrasing (evidence log wording)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("resolves both the category-click and leaf-click steps via the primitive, with no el.click() cascade fallback", async () => {
    ...
    const step10Params = baseParams(
      ...
      stagehand,
      `click the top-level category '${CATEGORY}' to expand its sub-options`,
      target
    );
    ...
    const step11Params = baseParams(
      page as unknown as Page,
      stagehand,
      `click the leaf option '${LEAF}' to commit the source value`,
      target
    );
    ...
    // No el.click()-fallback path executed for either step — the cascade
