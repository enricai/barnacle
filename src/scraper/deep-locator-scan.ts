 ...
 * that reads text + layout/visibility for EVERY match of an inner selector
 * inside the frame's own document, in a single CDP round-trip.
...
export interface FrameCandidateScanResult {
  ...
  index: number;
  /** Raw `element.textContent`, untrimmed — mirrors what `DeepLocatorDelegate.nth(index).textContent()` returns today. */
  text: string;
