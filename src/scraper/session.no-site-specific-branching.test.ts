...
/**
 * Guards CLAUDE.md's "site-agnostic core" rule for the deterministic
 * per-target init-script installer session.ts now calls into:
 * `installInitScriptOnAllFrames` must stay generic across every plugin/site,
 * never branching on a site or plugin identifier or naming a specific
 * vendor. This is a static-text property, not a runtime behavior, so it's
 * checked by scanning the source rather than by exercising the code.
 */
...
// they ever appeared in this path; the installer must only ever know about
// the generic per-target CDP install sequence, never a named vendor.
...
describe("deterministic per-target init-script installer stays site-agnostic (cdp-frame-init-script.ts)", () => {
  it("installInitScriptOnAllFrames contains no site/plugin-identifying branch", () => {
