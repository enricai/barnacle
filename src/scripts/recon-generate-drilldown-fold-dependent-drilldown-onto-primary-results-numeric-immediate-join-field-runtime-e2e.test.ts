...
import { buildMulticallSingleShotSearchDrillDownArrayWrappedNumericImmediateJoinFieldActionSteps } from "@/scripts/recon-generate-multicall-fixture";

const SEARCH_BODY = { results: [{ orderId: 101 }, { orderId: 102 }] };

const statusTokenFor = (orderId: number): string => `status-token-${orderId}`;
...
const STATUS_BODY_FOR = (orderId: number): string[] => [statusTokenFor(orderId)];
 ...
 * own NUMERIC join value inside a single-element request-body ARRAY
 * (`{"orderIds":[101]}`, a bulk-lookup shape) instead of as a flat top-level
 * scalar field.
 */
function stubArrayWrappedNumericImmediateJoinFieldFetch(): void {
  const fn = vi.fn().mockImplementation((_url: string, init?: { body?: string }) => {
    ...
    const responseBody = (() => {
      ...
      if (Array.isArray(requestBody.orderIds) && typeof requestBody.orderIds[0] === "number") {
        return STATUS_BODY_FOR(requestBody.orderIds[0] as number);
      }
      if (Array.isArray(requestBody.tokens) && typeof requestBody.tokens[0] === "string") {
        const statusToken = requestBody.tokens[0] as string;
        const orderId = Number(statusToken.replace("status-token-", ""));
        return ORDER_HISTORY_BULK_BODY_FOR(orderId);
      }
      throw new Error(
        `stubArrayWrappedNumericImmediateJoinFieldFetch: unrecognized request body ${JSON.stringify(requestBody)}`
      );
    })();
...
describe("recon-generate dependent drill-down fold executeHttp — array-wrapped numeric immediate join field runtime guard", () => {
  it("threads each primary item's own numeric join value into a bulk-lookup array instead of freezing it as an opaque caller payload", async () => {
    const steps =
    ...
    stubArrayWrappedNumericImmediateJoinFieldFetch();
    ...
    expect(data.results).toEqual([
      {
        orderId: 101,
        statusToken: "status-token-101",
        ts: "2024-10-03T00:00:02Z",
        event: "shipped",
      },
      {
        orderId: 102,
        statusToken: "status-token-102",
        ts: "2024-10-03T00:00:02Z",
        event: "shipped",
      },
    ]);
