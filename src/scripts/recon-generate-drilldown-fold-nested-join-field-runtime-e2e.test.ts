...
import { buildMulticallSingleShotSearchDrillDownNestedJoinFieldMultiItemActionSteps } from "@/scripts/recon-generate-multicall-fixture";
 ...
 * whose join key was resolved out of a NESTED field, not just a top-level one. */
...
describe("recon-generate drill-down fold executeHttp — nested join key runtime guard", () => {
  it("folds the drilled field onto EVERY primary item whose join key lives under a nested object", async () => {
    const actionSteps =
    ...
    expect(result.data).toEqual({
      results: [
        { identifiers: { sku: "sku-a" }, sku: "sku-a", amount: 19.99 },
        { identifiers: { sku: "sku-b" }, sku: "sku-b", amount: 24.99 },
      ],
    });
