...
import { CdpTransportClosedError, SessionTimeoutError, StepVerificationError } from "@/scraper/errors";
...
describe("recon-browser/persistReplannedFlow", () => {
  ...
  it("degrades to today's behavior (no extra keys, no throw) when the original bytes aren't valid JSON", () => {
    ...
    const parsed = JSON.parse(readFileSync(flowPath, "utf8"));
    expect(parsed).toEqual({ steps: ["Step A"] });
  });
