 ...
 * stale. The genuine residual gap — asserted below — is that
 * `buildRateLimitProbeTargets` had no `isNoiseUrl` guard of its own, so a
 * noise URL that reached `replays` by some other path had nothing stopping
 * it from becoming a rate-limit target. Do not re-file the end-to-end claim.
 */
...
describe("buildRateLimitProbeTargets — noise host exclusion", () => {
