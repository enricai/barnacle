/**
 * End-to-end acceptance coverage for the reported missing-ancestor-flag bug:
 * when the FAILED step itself carries no `submitStep` flag at all (not even
 * `submitStep: false` — the flag is simply absent from the flow JSON), a
 * solo replan bridge step that is itself genuinely submit-shaped must still
 * resume with `submitStep: true`. There is no ancestor flag to inherit here;
 * the flag must come purely from the bridge step's own text, per
 * `applyFailedStepFlagsToResumingBridgeStep` (recon-browser.ts). Drives
 * `main()` for real (only `createBrowserSession` and the replan LLM call are
 * stubbed) with a step that fails its cascade, has the replan LLM propose a
 * single submit-shaped bridge step, and asserts the next
 * `executeStepWithHealing` call — for the spliced-in bridge step — carries
 * `submitStep: true`.
 */
...
const ORIGINAL_STEP = "Click the 'Next' button";
const BRIDGE_STEP = "Click the 'Place Order' button to submit the purchase";
...
describe("recon-browser/main — submit-shaped replan bridge step with no ancestor submitStep flag", () => {
  const ORIGINAL_ARGV = process.argv;
  let runsRoot: string;

  beforeEach(() => {
    runsRoot = mkdtempSync(join(tmpdir(), "recon-browser-replan-noancestor-"));
    process.env.RECON_RUN_ID = "20260925-000000-replannoancestor";
  ...
  it("resumes with submitStep:true for a submit-shaped bridge step even though the failed step never declared submitStep", async () => {
    ...
    // No ancestor submitStep flag exists to inherit; the flag must be
    // derived purely from the bridge step's own submit-shaped wording.
