...
import { CALL_TYPE_JUDGE_MODAL_PRIORITY } from "@/lib/telemetry/call-types";
...
export interface JudgeModalPriorityInput {
  ...
  observations: { description: string; selector: string }[];
}

function buildModalPriorityPrompt(input: JudgeModalPriorityInput): string {
 ...
 */
export async function judgeModalPriorityWithLLM(params: {
  ...
  const result = await callHaikuJudge({
    ...
    schema: MODAL_PRIORITY_SCHEMA,
    callType: CALL_TYPE_JUDGE_MODAL_PRIORITY,
    captureFn,
  });
