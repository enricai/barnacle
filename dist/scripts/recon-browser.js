...
exports.isSubmitRevealedInvalid = isSubmitRevealedInvalid;
exports.hasBillingErrorBeenLogged = hasBillingErrorBeenLogged;
exports.resetBillingErrorFlagForTests = resetBillingErrorFlagForTests;
exports.logBillingErrorIfPresent = logBillingErrorIfPresent;
...
exports.renderUnfocusedObserve = renderUnfocusedObserve;
exports.rephraseWithLLM = rephraseWithLLM;
...
const modal_priority_1 = require("../lib/llm/judges/modal-priority");
...
function wireNetworkCapture(page, counter, signalCounter, recentCaptures, recentCaptureMeta, getCurrentPhase, getCurrentPageOrigin) {
    ...
    const onFinished = async (params) => {
        ...
        try {
            const result = await page.sendCDP("Network.getResponseBody", {
        ...
        if (req.requestPostData) {
            ...
            const parsed = typeof decodedParams === "object" && decodedParams !== null
                ? decodedParams
                : null;
            if (parsed) {
        ...
        // body for forensics.
        const urlHash = (0, node_crypto_1.createHash)("sha1").update(req.url, "utf8").digest("hex").slice(0, 8);
        ...
        try {
            ...
            logger.warn(`capture-write skipped for ${req.url.slice(0, 80)}: ${err instanceof Error ? err.message : String(err)}`);
        }
...
async function snapshotPage(page, signalCounter) {
 ...
 */
function findRecentBackendError(params) {
 ...
 * clean skip.
 */
function findRecentPageTransition(params) {
 ...
 */
async function countNgInvalidContainers(page) {
    const expr = `(() => {
 ...
 */
function describeAttemptEffectSignals(pre, post, recentCaptureMeta, preMetaLength) {
    ...
    if (networkDelta === 0 && bytesDelta >= 500) {
        observations.push(`dom-grew-without-network: body +${bytesDelta}B, ${textChanged ? "visible text changed" : "visible text unchanged"}, 0 same-origin non-GET requests (likely client-side validation rendered errors instead of submitting)`);
    }
    if (networkDelta > 0 && windowCaptures.length > 0) {
        ...
        if (sameOriginNonGet.length === 0) {
            observations.push(`network-fired-but-only-tracking: ${windowCaptures.length} requests captured but none were non-GET same-origin (analytics beacons / third-party tracking, not a form submit)`);
        }
    }
...
function shouldSkipTechnique(params) {
    ...
    if (technique === "structured-click") {
        ...
        if (!anyXpathResolved) {
            return {
                skip: true,
                reason: "structured-click needs a prior xpath; no attempt has resolved a selector yet — skipping to next technique",
            };
        }
    }
    if (technique === "observe-act-exclude") {
        ...
        if (observeAct2 &&
            ...
            return {
                skip: true,
                reason: "observe-act-exclude re-runs the same observe with exclusions; prior observe-act returned 0 candidates — no new candidates to surface, skipping",
            };
        }
    }
 ...
 */
function isSubmitRevealedInvalid(params) {
    const { isFinalStep, requireSubmitEndpoint, resolvedMethod, effectSignals, preSubmitInvalidCount, postAttemptInvalidCount, } = params;
    if (!isFinalStep)
        return false;
    if (!requireSubmitEndpoint)
        return false;
    if (resolvedMethod !== "click")
        return false;
    if (!effectSignals.includes("dom-grew-without-network"))
        return false;
    if (postAttemptInvalidCount <= preSubmitInvalidCount)
        return false;
    return true;
}
/**
 ...
 */
async function rephraseWithLLM(client, originalStep, triedSelectors, observeCandidates, failureReasons, captureFn = call_capture_1.captureLlmCall, 
/**
...
priorAttempts) {
    ...
    const priorInstructionList = (priorAttempts ?? [])
        ...
        .map((a, i) => `${i + 1}. [${a.technique}] "${a.instruction}" → ${a.verdict ?? "(no verdict captured)"}`)
        .join("\n");
 ...
 */
function hasBillingErrorBeenLogged() {
 ...
 */
function resetBillingErrorFlagForTests() {
    billingErrorLoggedThisProcess = false;
}
function logBillingErrorIfPresent(err) {
    if ((0, call_capture_1.classifyLlmCallFailure)(err) !== "anthropic-billing")
        return false;
    if (billingErrorLoggedThisProcess)
        return true;
    ...
    logger.fatal("FATAL_BILLING: Anthropic API rejected the call due to insufficient credit balance. Cascade cannot heal further. Top up at https://console.anthropic.com/billing then re-run.");
    return true;
}
...
function denormalizeStep(step) {
    if (!step.optional && !step.upload)
        return step.instruction;
    const out = { step: step.instruction };
    if (step.optional)
        out.optional = true;
    if (step.upload)
        out.upload = true;
 ...
 */
async function renderUnfocusedObserve(observations, options) {
    ...
    const judgeVerdict = await (0, modal_priority_1.judgeModalPriorityWithLLM)({
        client,
        input: {
 ...
 */
function pairInvalidWithErrors(invalidFieldList, errorTextList) {
    if (invalidFieldList.length === 0 || errorTextList.length === 0)
        return [];
    const invalidLines = invalidFieldList.split("\n").filter((l) => l.trim().length > 0);
    const errorLines = errorTextList.split("\n").filter((l) => l.trim().length > 0);
    if (invalidLines.length === 0 || errorLines.length === 0)
        return [];
    const pairs = [];
    ...
    for (let i = 0; i < max; i++) {
        ...
        if (!(inv.includes("ng-touched") && inv.includes("ng-dirty")))
            continue;
        ...
        if (fieldLabel.length === 0 || errorText.length === 0)
            continue;
        pairs.push({
...
async function extractLivePageFormEvidence(page, options) {
    ...
    const [invalidVerdict, errorVerdict, interactiveTargets] = await Promise.all([
        ...
        extractInteractiveTargetsNearInvalid(page).catch(() => []),
    ]);
 ...
 */
async function extractInteractiveTargetsNearInvalid(page) {
    const expr = `(() => {
    const out = [];
 ...
 */
function extractSubmitFailureEvidence(recentCaptureFilenames, 
/**
...
ownBackendHostnames, capturesDir = recon_shared_1.CAPTURES_DIR, mode = "strict") {
    ...
    for (const filename of recentCaptureFilenames.slice(-20)) {
        ...
        seen.add(filename);
        try {
            const path = (0, node_path_1.join)(capturesDir, filename);
 ...
 */
function harvestFieldErrors(body) {
    ...
    if (Array.isArray(errorsArr)) {
        for (const e of errorsArr) {
            if (typeof e === "string")
                ...
                const message = typeof fr.message === "string"
                    ? fr.message
    ...
    for (const bag of fieldBags) {
        if (bag && typeof bag === "object" && !Array.isArray(bag)) {
            for (const [field, msg] of Object.entries(bag)) {
...
async function replanRemainingFlow(params) {
    ...
    const priorReplanList = priorReplans.length > 0
        ? priorReplans
            ...
            .join("\n\n")
        : "";
    const candidates = await (0, stagehand_guard_1.guardedObserve)(stagehand, undefined, { timeout: STEP_WATCHDOG_MS }, captureFn).catch(() => []);
...
async function tryUploadPrimitive(params) {
    ...
    try {
        ...
        logger.warn(`upload primitive: setInputFiles threw: ${(0, errors_1.toErrorMessage)(err)}`);
        return false;
    }
    ...
    while (performance.now() - startedAt < UPLOAD_NETWORK_TIMEOUT_MS) {
        if (signalCounter.n > networkCountBefore) {
            logger.info(`upload primitive: network activity detected post-setInputFiles (name=${fixture.name}, size=${fixture.buffer.length}b)`);
            return true;
        }
    ...
    logger.info(`upload primitive: file attached in DOM after setInputFiles (deferred-upload widget; name=${fixture.name}, filesLength=${attachedLength})`);
    return true;
}
/**
 ...
 */
async function dispatchJqueryChangeEvent(page, selector) {
 ...
 */
async function verifyDomEffect(page, action) {
    ...
    try {
        ...
        switch (method) {
            ...
            case "selectOptionFromDropdown": {
                ...
                return ((selected.value.length > 0 && selected.value.toLowerCase().includes(want)) ||
 ...
 */
function narrowInvalidFormControl(entry) {
    if (entry === null ||
        typeof entry !== "object" ||
        !("label" in entry) ||
        ...
        typeof entry.classSignature !== "string" ||
        typeof entry.emptyOrUnchecked !== "boolean") {
        return null;
    }
    const af = entry.autoFilled;
    const narrowedAutoFilled = af !== null &&
        typeof af === "object" &&
        af !== undefined &&
        "action" in af &&
        "value" in af &&
        typeof af.action === "string" &&
        typeof af.value === "string"
        ? af
        : null;
    return {
        ...entry,
...
async function probeStepBeforeAttempts(params) {
    ...
    try {
        const candidates = await (0, stagehand_guard_1.guardedObserve)(stagehand, step, { timeout: STEP_WATCHDOG_MS }, captureFn);
        if (candidates.length === 0) {
            logger.info(`step ${stepIndex + 1}: probe found 0 candidates — treating as absent (skip cascade, route to replan if required)`);
            return "absent";
        }
...
async function executeStepWithHealing(params) {
    const { stagehand, page, step, optional, upload, stepIndex, phase, signalCounter, recentCaptures, recentCaptureMeta, anthropic, logger, captureFn, resumeFixture, isFinalStep, submitEndpointPattern, submittedStateSelectors, requireSubmitEndpointMatch, successUrlFragments, successPageTitleHints, ownBackendHostnames, knownErrorClassPrefixes, trajectory, } = params;
    ...
    if (probeResult === "absent") {
        ...
        const unfocusedObserve = await (0, stagehand_guard_1.guardedObserve)(stagehand, undefined, { timeout: STEP_WATCHDOG_MS }, captureFn).catch(() => []);
        ...
        throw new errors_2.StepVerificationError(`step ${stepIndex + 1} (${step.slice(0, 60)}) probe found no candidates on page; see ${dumpPath}`, "probe-absent");
    }
    ...
    for (let attempt = 1; attempt <= MAX_STEP_ATTEMPTS; attempt++) {
        ...
        try {
            if (attempt === 1) {
                ...
                const result = await (0, stagehand_guard_1.guardedAct)(stagehand, step, { timeout: STEP_WATCHDOG_MS }, captureFn);
                record.actResultSuccess = result.success;
                record.actResultDescription = result.actionDescription;
                for (const action of result.actions ?? []) {
                    if (action.selector)
                        triedSelectors.push(action.selector);
                    if (!resolvedAction)
                ...
                if (candidates.length === 0) {
                    ...
                    const result = await (0, stagehand_guard_1.guardedAct)(stagehand, target, { timeout: STEP_WATCHDOG_MS }, captureFn);
                    record.actResultSuccess = result.success;
                    record.actResultDescription = result.actionDescription;
            ...
            return { resolved: true, isCheckable: true, checked: false, strategyUsed: null };
                    ...
                    try {
                        ...
                        record.errorMessage = `structured-click: probe threw ${(0, errors_1.toErrorMessage)(err)}`;
                    }
                }
            }
            else {
                ...
                if (!anthropic) {
                    ...
                    const candidates = await (0, stagehand_guard_1.guardedObserve)(stagehand, step, { timeout: STEP_WATCHDOG_MS }, captureFn).catch(() => []);
                    ...
                    const unfocused = await (0, stagehand_guard_1.guardedObserve)(stagehand, undefined, { timeout: STEP_WATCHDOG_MS }, captureFn).catch(() => []);
                    const submitFailureList = extractSubmitFailureEvidence(recentCaptures, ownBackendHostnames);
                    ...
                    const rephrased = await rephraseWithLLM(anthropic, step, triedSelectors, candidates, failureReasons, captureFn, livePageEvidence, unfocused, submitFailureList, priorAttemptsForPrompt);
                    if (!rephrased) {
                        record.errorMessage = "llm declined to rephrase or returned outcome=impossible";
                    }
                    else {
                        record.instruction = rephrased;
                        const result = await (0, stagehand_guard_1.guardedAct)(stagehand, rephrased, { timeout: STEP_WATCHDOG_MS }, captureFn);
                        record.actResultSuccess = result.success;
                        record.actResultDescription = result.actionDescription;
                        for (const action of result.actions ?? []) {
                            if (action.selector)
            ...
            record.errorMessage = `${(0, errors_1.toErrorMessage)(err)}${cause ? ` (cause: ${cause})` : ""}`;
            logBillingErrorIfPresent(err);
        }
        ...
        const isStateClass = resolvedAction !== null && STATE_CLASS_METHODS.has(resolvedAction.method ?? "");
        ...
        if (verified && requireSubmitEndpoint) {
            ...
            const unfocusedForJudge = await (0, stagehand_guard_1.guardedObserve)(stagehand, undefined, { timeout: STEP_WATCHDOG_MS }, captureFn).catch(() => []);
            ...
            if (judgeVerdict === null) {
                ...
                if (!requireSubmitEndpointMatch && domSubmittedMatch !== null) {
                    logger.info(`submit verified via submitted-state DOM selector '${domSubmittedMatch}' (judge unavailable)`);
                ...
                logger.info(`submit verified by judge: ${judgeVerdict.rationale} (dom=${judgeVerdict.dom_signal ?? "—"}, url=${judgeVerdict.url_signal ?? "—"})`);
        ...
        if (!verified &&
            ...
            if (xpath) {
                try {
                    ...
                    const probeResult = (await page.evaluate(clickExpr));
              ...
              return false;
                    ...
                    const checkboxStateVerified = probeResult.kind === "checkbox" &&
                    ...
                    let retryVerified = retryNetworkFired ||
                        retryUrlChanged ||
                        retryHtmlDelta !== 0 ||
                        retryTextChanged ||
                  ...
                  return null;
                        ...
                        if (judgeVerdict === null) {
                            ...
                            if (!requireSubmitEndpointMatch && domSubmittedMatch !== null) {
                                logger.info(`n+16 fallback submit verified via submitted-state DOM selector '${domSubmittedMatch}' (judge unavailable)`);
                                record.verifiedBy = "submitted-state-dom";
                            }
                            else {
                                retryVerified = false;
                                failureReasons.push(`n+16 fallback: submit-judge-unavailable: no DOM-state match either`);
                            }
                        }
                        else if (judgeVerdict.verified) {
    ...
    const finalObserve = await (0, stagehand_guard_1.guardedObserve)(stagehand, step, { timeout: STEP_WATCHDOG_MS }, captureFn).catch(() => []);
    const pageTitle = await page.title().catch(() => "");
    ...
    throw new errors_2.StepVerificationError(`step ${stepIndex + 1} (${step.slice(0, 60)}) failed verification after ${MAX_STEP_ATTEMPTS} attempts; see ${dumpPath}`, "cascade-exhausted");
}
...
async function main() {
    const { url, flow: rawFlow, flowFile, provider, resumeFixturePath, saveReplan, advancedStealth, dumpDomBeforeStep, allocateEmailEnvVar, submitEndpointPattern, submittedStateSelectors, requireSubmitEndpointMatch, successUrlFragments, successPageTitleHints, ownBackendHostnames, knownErrorClassPrefixes, originalShape, } = parseCli();
    ...
    try {
        ...
        if (requireSubmitEndpointMatch && ownBackendHostnames.length > 0) {
            ...
            if (auditFailed) {
                logger.error(`end-of-run audit FAILED: no captured 2xx had hostname in ${JSON.stringify(ownBackendHostnames)} — submission did not land despite verifier success`);
                // Exit non-zero so the runner counts this as a real failure rather
                // than rolling silent-pass forward as success.
                process.exit(1);
            }
            logger.info(`end-of-run audit PASSED: at least one captured 2xx matched submitEndpointPattern`);
        }
