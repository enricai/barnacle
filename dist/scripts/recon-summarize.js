...
const logger = (0, logging_1.getScriptLogger)("recon-summarize");
...
async function main() {
    ...
    for (const c of captures) {
        try {
            ...
            if (c.operationName)
                entry.operations.add(c.operationName);
    ...
    const rateLimitSummary = rateLimitRaw.length === 0
        ...
        : rateLimitRaw
            ...
            .join("\n");
    const headerCounts = (0, recon_shared_1.tallyResponseHeaders)(replays);
    const successCount = replays.filter((r) => r.success).length;
    const hazards = [];
    if (has401)
        hazards.push("Auth required on some endpoints (401)");
    if (has403)
        hazards.push("Bot detection active on some endpoints (403) — may need more headers or Stagehand-only");
    ...
    const lines = [
        ...
        has401
            ? replays
                ...
                .map((r) => `- \`${r.url}\` — 401. Capture token, determine lifetime, build refresh strategy.`)
                .join("\n")
        ...
        auxFiles.length === 0
            ...
            : [
                `_Downloaded to \`${recon_shared_1.AUX_DIR}\` — commit each as a static fixture in \`src/sites/<id>/fixtures/\`._`,
                ``,
    ...
    (0, node_fs_1.writeFileSync)(outPath, content);
    logger.info(`findings written to ${outPath}`);
}
main().catch((err) => {
    logger.error(`recon-summarize failed: ${(0, errors_1.toErrorMessage)(err)}`);
    process.exit(1);
});
//# sourceMappingURL=recon-summarize.js.map