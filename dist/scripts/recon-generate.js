...
function emitMultiStepExecuteHttp(actions, inputBody, errorSignals, fieldNameMap, outDiscoveredFields, fieldOptionsMap, outDiscoveredOptionFields, outDiscoveredRawOptionFields, outDiscoveredAdditionalBodyKeys, baseUrl, baseUrlDerivedHeaders, tenantSubdomainHeaders) {
    ...
    for (let i = 0; i < actions.length; i++) {
        ...
        if (step.isMultipart) {
            ...
            lines.push(`    // Expected response shape: ${JSON.stringify(summariseResponseShape(cap.responseBody))}`, `    const ${fdVar} = new FormData();`, `    const ${fdVar}_bytes = Uint8Array.from(payload.Resume);`, `    ${fdVar}.append("files[]", new Blob([${fdVar}_bytes], { type: payload.ResumeContentType }), payload.ResumeFilename);`, `    const ${headersVar} = { ...Object.fromEntries(Object.entries(BASE_HEADERS).filter(([k]) => k.toLowerCase() !== "content-type"))${perCallHeadersLit} };`, `    const ${respVar} = await fetch(\`${r.url}\`, {`, `      method: "POST",`, `      headers: ${headersVar},`, `      body: ${fdVar},`, `    });`, `    if (!${respVar}.ok) throw new Error(\`step ${step.varName} (multipart upload) failed: HTTP \${${respVar}.status}\`);`);
            if (bindResponse) {
...
function emitContractTs(opts) {
    const { siteId, pascal, baseUrl, baseHeaders, minTime, safeRps, responseBody, gql, gqlQuery, endpointPath, auxFiles, multiStepBody, inputBody, hasMultipartStep = false, discoveredFormFields, fieldOptionsMap, discoveredOptionFields, discoveredRawOptionFields, discoveredAdditionalBodyKeys, } = opts;
...
function emitBrowserFlowTs(opts) {
    ...
    const actCalls = flowSteps.length > 0
        ? flowSteps
            .map((step) => {
...
async function main() {
    ...
    if ((0, node_fs_1.existsSync)(outDir) && !force) {
    ...
    const flowSteps = (() => {
        ...
        try {
            return JSON.parse((0, node_fs_1.readFileSync)(flowFile, "utf8"));
        }
        catch {
            return [];
        }
    ...
    (0, node_fs_1.writeFileSync)(`${outDir}/flows/browser-flow.ts`, emitBrowserFlowTs({ siteId, pascal, baseUrl, flowSteps, isSubmissionFlow }));
