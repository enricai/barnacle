...
exports.getSuperCategoryPricing = getSuperCategoryPricing;
exports.getCategoryPricing = getCategoryPricing;
exports.getGroupPricing = getGroupPricing;
const date_fns_1 = require("date-fns");
const envelope_1 = require("@/api/helpers/envelope");
const response_cache_1 = require("@/cache/response-cache");
const pricing_1 = require("@/scraper/flows/pricing");
...
async function scrapeAndPersist(request, granularity) {
    const cabins = await (0, pool_1.runWithSession)((session) => (0, pricing_1.scrapeSailingPricing)(session, {
    ...
    await (0, store_1.savePricingSnapshot)({
        brandCode: request.brandCode,
        shipCode: request.shipCode,
        sailDate: (0, date_fns_1.parseISO)(request.sailDate),
        packageCode: request.packageCode,
        currencyCode: request.currencyCode,
        occupancy: request.occupancy,
        bookingTypeCode: request.bookingTypeCode,
 ...
 */
async function getSuperCategoryPricing(request) {
    ...
    const superCategoryBestPrices = Array.from(bySuper.entries()).map(([code, rows]) => {
        const seed = rows[0];
        if (!seed)
            throw new Error(`empty super-category bucket for ${code}`);
 ...
 */
async function getCategoryPricing(request) {
 ...
 */
async function getGroupPricing(request) {
